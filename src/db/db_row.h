...
public:
    ...
    Row(const string& name, const string& macro = "", const int x = 0, const int y = 0, const unsigned xNum = 0, const unsigned yNum = 0, const bool flip = false, const unsigned xStep = 0, const unsigned yStep = 0)
        : _name(name)
        , _macro(macro)
        , _x(x)
        , _y(y)
    ...
    int getSiteT(int dbLY, int siteH) const { return getSiteT(dbLY, siteH) + _yNum; }
