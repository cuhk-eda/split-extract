...
namespace util{
    template<typename T> inline void minmax(const T v1, const T v2, T &minv, T &maxv){
        if(v1 < v2){
            minv = v1;
            maxv = v2;
        }else{
    ...
    template<typename T> inline void bounds(const T v, T &minv, T &maxv){
        if(v < minv){
            minv = v;
        }else if(v > maxv){
            maxv = v;
...
inline int binContainedL(int lx, int blx, int bhx, int binw){
    return (std::max(blx, lx) - blx + binw - 1) / binw;
}
inline int binContainedR(int hx, int blx, int bhx, int binw){
    return (std::min(bhx, hx) - blx) / binw - 1;
}
inline int binOverlappedL(int lx, int blx, int bhx, int binw){
    return (std::max(blx, lx) - blx) / binw;
}
...
////////////INTEGER PACKING/////////////////
inline long long packInt(const int x, const int y){
...
inline int getrand(int lo, int hi){
    return (rand()%(hi-lo+1))+lo;
}
inline double getrand(double lo, double hi){
    return (((double)rand()/(double)RAND_MAX)*(hi-lo)+lo);
}

//////////////BIT MANIPULATION///////////////////////

template<typename T> inline void setBit(T &val, T bit){ val |= bit; }
template<typename T> inline void unsetBit(T &val, T bit){ val &= (~bit); }
template<typename T> inline void toggleBit(  T &val, T bit){ val ^= bit; }
template<typename T> inline bool isSetBit(   T val, T bit){  return (val&bit)>0; }
template<typename T> inline T    getBit(     T val, T bit){  return val & bit;   }
